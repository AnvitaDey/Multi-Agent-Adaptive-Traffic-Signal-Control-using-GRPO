"""
training/train_ppo_multiagent.py  (v3 — mixed-density, TRUE resume support)
...
def parse_steps_from_ckpt(ckpt_path: str) -> int:
    """
    Safely extract the step count from an SB3 checkpoint filename.

    SB3 names checkpoints like:  ppo_a2_200000_steps.zip
    We work only on the basename to avoid misreading underscores in
    directory names (e.g. 'checkpoints_a2').
    """
    fname = os.path.basename(ckpt_path)          # e.g. "ppo_a2_200000_steps.zip"
    fname = fname.replace(".zip", "")             # "ppo_a2_200000_steps"
    parts = fname.split("_")                      # ["ppo", "a2", "200000", "steps"]
    # The step count is always the second-to-last token ("steps" is last)
    try:
        return int(parts[-2])
...
def main():
    ...
    # CSV log (append so resume doesn't wipe previous rows)
    log_file     = os.path.join(log_dir, "training_log.csv")
    write_header = not os.path.exists(log_file)
    with open(log_file, "a", newline="") as f:
        if write_header:
            csv.writer(f).writerow(
                ["tls_id", "timesteps", "ep_rew_mean", "ep_len_mean", "value_loss"]
            )
    ...
    for tls in todo_agents:
        ...
        # ──────────────────────────────────────────────────────────
        # RESUME LOGIC
        # Look for the latest checkpoint saved by CheckpointCallback.
        # SB3 names them:  ppo_<tls>_<steps>_steps.zip
        # ──────────────────────────────────────────────────────────
        ckpts = sorted([
        ...
        if ckpts:
            ...
            print(f"  🔁 Resuming from checkpoint: {ckpts[-1]}")
            print(f"     Steps done : {steps_done:,}  |  Remaining: {remaining:,}")
        ...
        # ──────────────────────────────────────────────────────────
        # TRAIN (or skip if nothing left)
        # reset_num_timesteps=False  → internal counter keeps going,
        # so CheckpointCallback saves at the right cumulative steps.
        # ──────────────────────────────────────────────────────────
        if remaining > 0:
        ...
        print(f"  --> Saved final model: {final_dir}/{tls}.zip")
