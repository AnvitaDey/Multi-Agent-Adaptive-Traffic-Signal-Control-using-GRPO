...
class TrafficEnv(gym.Env):
    ...
    def _probe_obs_dim(self):
        ...
        traci.start([binary, "-c", self.sumo_cfg, "--no-warnings", "true", "--time-to-teleport", "300"], label=probe_label)
        ...
        obs_dim = len(lanes) * 2 + num_phases
        print(f"[TrafficEnv:{self._label}] obs_dim={obs_dim} ({len(lanes)} lanes x 2 + {num_phases} phases)")
        return obs_dim
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self._sumo_running:
            ...
            self._sumo_running = False
        binary = "sumo-gui" if self.use_gui else "sumo"
        traci.start([binary, "-c", self.sumo_cfg, "--no-warnings", "true", "--time-to-teleport", "300"], label=self._label)
        traci.switch(self._label)
    ...
    def step(self, action):
        ...
        if terminated or truncated:
            traci.close()
            self._sumo_running = False
        return self._get_obs(), reward, terminated, truncated, {**metrics_after, "phase": self._phase_index}
    ...
    def _get_obs(self):
        ...
        ph = np.zeros(self._num_phases, dtype=np.float32)
        ph[self._phase_index % self._num_phases] = 1.0
        return np.concatenate([np.array(q, dtype=np.float32), np.array(w, dtype=np.float32), ph])
